...
contract LoanLogic is LoanStorage, ReentrancyGuardUpgradeable,Modifiers {
    ...
    ) public initializer ZeroAddress(_usdcTokenAddress) ZeroAddress(_clethTokenAddress) ZeroAddress(_priceFeedAddress){
    ...
    function calculateMaxLoanAmount(
        uint256 clethAmount
    ) ZeroAmount(clethAmount) public view  returns (uint256) {
     ...
     */
    function repayLoan(uint256 loanId) LoanIdNotExits(loanId) public nonReentrant {
    ...
    function calculateRepaymentAmount(
        uint256 loanId
    ) LoanIdNotExits(loanId) public view returns (uint256) {
        Loan memory loan = loans[loanId];
    ...
    function liquidateCollateral(uint256 loanId) LoanIdNotExits(loanId) public nonReentrant {
        Loan storage loan = loans[loanId];
    ...
    function transferTokensFrom(
        ...
        require(token.transferFrom(from, address(this), amount),"trensfer from failed");
    }
}
