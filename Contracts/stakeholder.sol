...
import "./IFigmentEth2Depositor.sol"; // Import the interface
import "./Staking.sol"; 
...
contract StakeHolder {
    ...
    function depositToFigment(
        ...
        require(address(this).balance >= depositAmount, "Insufficient balance for deposit");
    ...
    modifier onlyMasterOwner() {
        require(StakingMaster(masterContract).isOwner(msg.sender), "Caller is not the master owner");
        _;
    }

    
    function receiveCleth(uint256 amount) external {
        require(clethToken.transferFrom(msg.sender, address(this), amount), "Transfer failed");
